...
if($_SESSION['login'])
{
    ...
    if(isset($_POST['submit']))
    {
        ...
        if($answer == "nessie")
        {
            ...
            $s = "UPDATE `users` SET `lastlevel`= 6 ,`time` = '$timestamp'  WHERE `username` = '$username'";
            echo date("Y-m-d h:i:s");
            $r = mysqli_query($conn , $s);
            header("Location: http://{$_SERVER['HTTP_HOST']}/WebQuest2/Users/lastpage.php");
        }
    }
...
<h5 class="help" style="color:white; border:1px solid white; padding:15px; width:10%; margin:auto auto; cursor:help"> HINT </h5>
        <div style="color:white margin:top:20px; padding:20px; background-color:black;" class="hints jumbotron tex-center"> MONSTER    </div>
